...
class emailController {
    static async emailList(req, res) {
        try {
            const sqlEmailList = db.query('select tb_email.*, tb_group.group_name from tb_email inner join tb_group on tb_email.group = tb_group.id where is_active = 1', function(err, rows) {
                if(rows.length > 0) {
                    res.json({
                        statusCode: 200,
    ...
    static async getEmail(req, res) {
        try {
            ...
            const sqlGetEmailById = db.query('select * from tb_email where id = ? and is_active = 1', [id], function(err, row) {
                if(row) {
                    res.json({
                        statusCode: 200,
    ...
    static async addEmail(req, res) {
        try {
            const { name, email, groupId } = req.body
             if(req.body) {
                 const sqlAddEmail = db.query('insert into tb_email (name, email, `group`) values (?,?,?)', [name, email, groupId], function(err, row) {
                     if(row) {
                        res.json({
                            statusCode: 200,
    ...
    static async addBulkEmail(req, res) {
        try {
            ...
            await importToDB(path.join(process.cwd(), '/upload/', excel_file.filename))
    ...
    static async editEmail(req, res) {
        try {
            ...
            const sqlEditEmail = db.query('update tb_email set name = ?, email = ?, `group` = ? where id = ?', [name, email, group, id], function(err, row) {
                if(row) {
                    res.json({
                        statusCode: 201,
    ...
    static async deactivateEmail(req, res) {
        try {
            ...
            const sqlDeactivateEmail = db.query('update tb_email set is_active = 0 where id = ?', [id], function(err, row) {
                if(row) {
                    res.json({
                        statusCode: 201,
