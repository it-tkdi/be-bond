...
class authController {
      ...
      const sqlGetData = db.query(
        ...
        async function (err, rows) {
          ...
          if (rows.length > 0 && comparedPassword) {
            ...
            const token = jwt.sign(cleanData, process.env.TOKEN_SECRET, {
              expiresIn: "2h",
            });
            cleanData.access_token = token;
            const sqlInsertToken = db.query(
              "update login set access_token = ? where id = ?",
              [token, cleanData.id],
              function (err, rows) {
                if (rows) {
                  res.json({
  ...
  static async changePassword(req, res) {
    try {
      ...
      const sqlChangePassword = db.query(
