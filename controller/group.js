...
class groupController {
  static async addGroup(req, res) {
    ...
    try {
      const checkGroupName = db.query(
        "select * from tb_group where group_name = ?",
        [groupName],
        function (err, rows) {
          if (err) {
            res.json({
            ...
            if (rows.length > 0) {
              ...
              const sqlInsertGroupName = db.query(
  ...
  static async groupList(req, res) {
    try {
      const sqlGetGroupList = db.query(
  ...
  static async getGroup(req, res) {
    try {
      ...
      const sqlGetGroup = db.query(
        "select * from tb_group where id = ?",
        [id],
        function (err, row) {
          if (err) {
            res.json({
