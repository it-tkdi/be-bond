...
exports.sendEmail = async (emailData, imageFile) => {
    const transporter = nodemailer.createTransport({
      ...
      recipients.forEach(async function (toArr, i, recipients) {
        db.query('select * from tb_email where email = ?', [recipients[i]], async function(err, row) {
          if(row) {
            let msg = {
              from: process.env.EMAIL_ADDRESS, // sender address
              subject: emailData.subject, // Subject line
              html: emailTemplate.newsLetter(recipients[i], row[0].name, emailData, imageFile) // html body
          }
          msg.to = toArr;
